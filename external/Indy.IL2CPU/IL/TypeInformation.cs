...
namespace Indy.IL2CPU.IL {
	...
	public class TypeInformation {
		public struct Field {
			...
			public Field(int aSize, bool aNeedsGC, TypeDefinition aFieldType, bool aIsExternalField) {
				NeedsGC = aNeedsGC;
		...
		public readonly bool NeedsGC;
		public TypeInformation(uint aStorageSize, Dictionary<string, Field> aFields, TypeReference aTypeDef, bool aNeedsGC) {
			Fields = aFields;
			StorageSize = aStorageSize;
			TypeDef = aTypeDef.Resolve();
			NeedsGC = aNeedsGC;
		}

		public override string ToString() {
			StringBuilder xResult = new StringBuilder();
			xResult.AppendLine(String.Format("Type '{0}'", TypeDef.Module.Assembly.Name));
			...
			if (Fields.Count == 0) {
				...
				foreach (KeyValuePair<string, Field> xField in (from item in Fields
																...
																select item)) {
					xResult.AppendLine(String.Format("\t'{0}': '{1}'", xField.Key, xField.Value.FieldType.FullName));
