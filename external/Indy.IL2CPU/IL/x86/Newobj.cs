...
namespace Indy.IL2CPU.IL.X86
{
    ...
    public class Newobj : Op
    {
        ...
        public override void DoAssemble()
        {
            Assemble(Assembler,
                     Constructor,
                     Engine.RegisterType(Constructor.DeclaringType),
                     CurrentLabel,
                     MethodInformation,
            ...
            string aNextLabel)
        {
            if (ctorDef != null)
            {
                Engine.QueueMethod(ctorDef);
            }
            else
            {
                throw new ArgumentNullException("aCtorDef");
            }
            var xTypeInfo = Engine.GetTypeInfo(ctorDef.DeclaringType);
            MethodInformation xCtorInfo = Engine.GetMethodInfo(ctorDef,
                                                                   ctorDef,
                                                                   Label.GenerateLabelName(ctorDef),
                                                                   Engine.GetTypeInfo(ctorDef.DeclaringType),
                                                                   aCurrentMethodInformation.DebugMode);
            if (xTypeInfo.NeedsGC)
            {
                uint xObjectSize = ObjectUtilities.GetObjectStorageSize(ctorDef.DeclaringType);
                ...
                uint xObjSize = 0;
                int xGCFieldCount = (from item in Engine.GetTypeFieldInfo(ctorDef,
                                                                          out xObjSize).Values
                                     where item.NeedsGC
                                     select item).Count();
                ...
                new Assembler.X86.Call { DestinationLabel = Label.GenerateLabelName(ctorDef) };
                new Test { DestinationReg = Registers.ECX, SourceValue = 2 };
